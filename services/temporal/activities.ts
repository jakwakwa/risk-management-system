...
const vertexAI = new VertexAI({
	project: projectId || process.env.GCP_PROJECT_ID || "stratcol-risk-analysis-engine", // Fallback to avoid crash if env missing
...
export async function screenCandidate(jobId: string): Promise<ScreenResult> {
  ...
  `;
	...
	try {
		...
		vectorMatches = await db.$queryRaw<
      ...
      `;
		...
		console.warn(
			"Vector search failed (likely no GCP creds or configured correctly):",
			err
		);
	...
	const result =
		sortedMatches.length > 0 && sortedMatches[0].score >= 0.7 ? "MATCH" : "CLEAR";
...
): Promise<{ reportUrl: string; pdfHash: string }> {
...
export async function auditLog(
	jobId: string,
	screenResult: ScreenResult,
	reportData: { reportUrl: string; pdfHash: string }
): Promise<void> {
