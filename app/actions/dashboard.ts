...
export async function getDashboardData() {
	try {
      ...
      `,
		...
		const clients = bqClients.map((bqData: BQTransactionAgg) => {
			...
			return {
				...
				bounceRate: 0,
				lastReview: bqData.last_seen
					? new Date(bqData.last_seen.value).toISOString().split("T")[0]
					: "N/A",
			};
		});
		...
		return {
			...
			alerts: recentAlerts.map(alert => ({
				id: alert.id,
				client:
					alert.riskProfile?.monitoringJob?.clientName &&
					alert.riskProfile?.monitoringJob?.type === "CLIENT_MONITORING"
						? (() => {
								try {
